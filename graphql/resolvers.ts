...
export const resolvers = {
  ...
  Mutation: {
    createHome: async (
      _parent: unknown,
      args: { houseNo: string; address: string },
      context: GraphQLContext
    ) => {
      try {
        const userEmail = requireUserEmail(context);
    ...
    inviteUserToHome: async (
      _parent: unknown,
      args: { homeId: string; email: string },
      context: GraphQLContext
    ) => {
      try {
        const userEmail = requireUserEmail(context);
    ...
    promoteHomeMemberToAdmin: async (
      _parent: unknown,
      args: { homeId: string; email: string },
      context: GraphQLContext
    ) => {
    ...
    createCategory: async (
      _parent: unknown,
      args: { name: string; homeId: string },
      context: GraphQLContext
    ) => {
      try {
        const userEmail = requireUserEmail(context);
  ...
  Home: {
    owners: (parent: { owners?: string[]; ownerEmail?: string | null }) => {
      return getHomeOwners(parent);
    },
    pendingInvites: (parent: { pendingInvites?: string[] }) => {
      return getPendingInvites(parent);
    },
